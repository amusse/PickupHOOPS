...
def makeTeams(game_players):
	...
	return [game_players[0],game_players[3],game_players[5],game_players[7], game_players[9]], 
		[game_players[1],game_players[2],game_players[4],game_players[6],game_players[8]]
...
for POP_SIZE in range(1000, 1500, 500):
	...
	for NUM_TRIALS in range(500, 25001, 1):
		counter += 1
		# Method 1:
		...
		fit = stats.norm.pdf(player_ratings, numpy.mean(player_ratings), numpy.std(player_ratings))  #this is a fitting indeed

		plt.plot(player_ratings,fit,'--r')
		n, bins, patches = plt.hist(player_ratings, normed=True, bins=40)
		plt.axis([550, 1450, 0, 0.015])
		plt.xlabel("Player Rating")
		plt.ylabel("Frequency (normalized)")
		plt.title("Method 1: Pop size {}, Games played {}".format(POP_SIZE, NUM_TRIALS))
		plt.savefig("METHOD1_POP{}_GAME{}.png".format(POP_SIZE, NUM_TRIALS), bbox_inches='tight',dpi=100)
		plt.close()
		...
		print("Games played:    {}".format(NUM_TRIALS))
		...
		# Method 2: each player gains a weighted average of their team's rating gainPOP_SIZE = 1000
		player_ratings = [1000]*POP_SIZE
		games_played = [0]*POP_SIZE
		team1 = []
		team2 = []
		...
		# METHOD 2
			...
			for player in team1:
				games_played[player[1]] += 1
				player_gain = ((1.0/5.0) / (player_ratings[player[1]]/team1_avg) * calcPlayerRatingGain(team1_gain, games_played[player[1]]))
				player_ratings[player[1]] += player_gain

			for player in team2:
				games_played[player[1]] += 1
				player_gain = ((1.0/5.0) / (player_ratings[player[1]]/team2_avg) * calcPlayerRatingGain(team2_gain, games_played[player[1]]))
				player_ratings[player[1]] += player_gain
		...
		plt.plot(player_ratings,fit,'--r')
		n, bins, patches = plt.hist(player_ratings, normed=True, bins=40)
		plt.axis([900, 1100, 0, 0.075])
		plt.xlabel("Player Rating")
		plt.ylabel("Frequency (normalized)")
		plt.title("Method 2: Pop size {}, Games played {}".format(POP_SIZE, NUM_TRIALS))
		plt.savefig("METHOD2_POP{}_GAME{}.png".format(POP_SIZE, NUM_TRIALS), bbox_inches='tight',dpi=100)
		plt.close()
		...
		# Method 3: each players gains an inversely weight average of their team's rating gain
		player_ratings = [1000]*POP_SIZE
		games_played = [0]*POP_SIZE
		team1 = []
		team2 = []
		...
		plt.xlabel("Player Rating")
		plt.ylabel("Frequency (normalized)")
		plt.title("Method 3: Pop size {}, Games played {}".format(POP_SIZE, NUM_TRIALS))
		plt.savefig("METHOD3_POP{}_GAME{}.png".format(POP_SIZE, NUM_TRIALS), bbox_inches='tight',dpi=100)
		plt.close()
